...
1. Создает работника через `auth-service`.
2. Создает оборудование.
3. Создает шаблон чек-листа.
4. Создает маршрут.
5. Назначает обход работнику по `employee_id`.
...
Для административных роутов нужен токен начальника:
