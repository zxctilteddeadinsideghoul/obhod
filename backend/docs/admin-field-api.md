...
Из ответа нужно взять `access_token` и передавать его как `<admin_access_token>`.

## Создать работника для назначения обхода

Перед назначением обхода работника можно создать через `auth-service`:
...
`employee_id` из ответа используется при создании обхода в поле `employee_id`.
